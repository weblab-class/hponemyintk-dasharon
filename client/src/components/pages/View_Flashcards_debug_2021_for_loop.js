...
cleanAnnotInput = (initAnnotInput) => {
  ...
  let annot_to_clean = JSON.parse(JSON.stringify(initAnnotInput));; //this makes a copy can edit from lecture  onJavascript perhaps, ref is https://www.samanthaming.com/tidbits/50-how-to-deep-clone-an-array/
    ...
    //This for loop only edits the first entry. It will print out information on each entry and can be used for debugging. On 1/15 at 21:50 it seemed like this was due to the console's printing timing- variable names' getting changed ahead of when they should have led to this debugging. When we only edit one variable name others don't change though, so think is is a console logging issue
    ...
    //PLEASE CHANGE ME BACK! initAnnotInput.length for < condition
    for (let annot_to_add = 0; annot_to_add < 2; annot_to_add++)
      {
        console.log("STARTING which iteration?")
        console.log(annot_to_add);
        let new_annot_init = annot_to_clean[annot_to_add];
        console.log("ANNOTATION EDITING");
        console.log(new_annot_init.geometry); //initial annotation
        ...
        console.log("EDITING INFO FOR THIS ANNOTATION")
        }
        ...
        console.log(new_annot_init.geometry); //edited annotation

...
router.post("/photo_simple_w_annotate", auth.ensureLoggedIn, (req, res) => {
  ...
  uploadImagePromise(req.body.photo_placeholder).then( (image_upload_info) => {//if success then set up schema //If not error continue [ref: https://stackoverflow.com/questions/30469261/checking-for-typeof-error-in-js]
...
router.get("/photosimpletestOne", auth.ensureLoggedIn, async (req, res) => {
  console.log("api.js photosimpletestOne req is:::",req.query);
  try{
